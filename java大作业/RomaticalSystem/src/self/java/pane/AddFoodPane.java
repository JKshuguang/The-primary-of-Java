...
public class AddFoodPane extends JPanel {
	 ...
	 * Create the panel.
	 */
	public AddFoodPane(JFrame lastFrame,JFrame jFrame,Dimension d, Image image) {
        super();
        //得到当前Frame
        this.jFrame = jFrame;
        this.d = d;
	...
	private void handleActionEvent() {
		...
		commitButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String placeString = placeField.getText().trim();
				String addString = picAddress.getText().trim();
				
				//写SQL
				String sql = "Insert into FoodsTable values(?,?,?,?)";
				
				//执行SQL
				dao.update(sql, no,desString,placeString,addString);
				
				
				//给简单描述生成ItemPane
				ItemPane itemPane = new ItemPane(no,desString);
				FoodsPane.center_pane.add(itemPane);
				FoodsPane.itemPaneList.add(itemPane);
